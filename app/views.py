...
ALPHANUMERIC_0 = list('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789')
    ...
    def do_get_wikipedia_extract(searchphrase, lang=None):
        ...
        if not r.status_code == 200:
            raise UnexpectedStatusCodeException(
                'Received an HTTP %d response code!')
        else:
            return r
    ...
    try:
        ...
        if page_not_found(r):
            raise PageNotFoundException(
                'Could not find "%s" in Wikipedia (%s). Please try another topic.' % (searchphrase, lang))
        else:
            return get_text(r)
...
def get_line(searchphrase):
    ...
    if len(lines) < 1:
        raise NoSuitableSentenceException(
            'Could not find a suitable sentence for %s.' % searchphrase)
...
def get_cryptogram(line):
    ...
    ALPHANUMERIC_1 = copy.deepcopy(ALPHANUMERIC_0)
    random.shuffle(ALPHANUMERIC_1)
    mapper = dict(zip(ALPHANUMERIC_0, ALPHANUMERIC_1))
